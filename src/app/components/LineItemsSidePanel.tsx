...
export default function LineItemsSidePanel({ title, items }: LineItemsSidePanelProps) {
      ...
      <div className="p-6">
        ...
        <div className="space-y-1.5">
          {items.map(item => (
            <div
              key={item.id}
              className={`border ${expandedItems.includes(item.id) ? 'border-2 border-blue-500 hover:border-blue-500' : 'border-gray-300 hover:border-blue-500'} rounded bg-white hover:bg-gray-50 transition-colors`}
            >
              <div className="p-4">
                ...
                {expandedItems.includes(item.id) && item.extraDetails}
              </div>
